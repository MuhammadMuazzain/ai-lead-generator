...
<div>
    <h1 style="margin: 0; padding: 0; font-size: 2.5rem; font-weight: bold;">
        <span style="font-size:2.5rem;">🎯</span> AI-Powered Lead Generation Agent
    </h1>
    <p style="margin-top: 0.25rem; color: #666;">
        by <a href="https://github.com/MuhammadMuazzain" target="_blank">Muhammad Muazzain</a>
        · powered by OpenAI
    </p>
</div>
"""
...
with st.sidebar:
    st.caption("Project by Muhammad Muazzain")
    bright_data_api_key = st.text_input("Enter your scraping API key", type="password")
    st.divider()
    ...
    2. **Submit your query** to start the AI agent
...
def fetch_leads_from_brightdata(filters):
    """Fetch real leads via the configured scraping API."""
    if not bright_data_api_key:
        st.error("Scraping API key is required")
        return []
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"Scraping API error: {response.status_code} - {response.text}")
            return [
            ]
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        message_placeholder.markdown("📊 Fetching leads from scraping API...")
